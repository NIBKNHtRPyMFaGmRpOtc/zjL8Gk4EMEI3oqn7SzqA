...
local chosenGame = ({
    ...
    [4032944086] = "workoutisland.lua"

})[game.PlaceId]
getgenv().feetie = false

if chosenGame then
    getgenv().feetie = true 
    if getgenv().feetie == true then
        wait(1)
        Barload:TweenSize(UDim2.new(0, 100,0, 2), Enum.EasingDirection.Out, Enum.EasingStyle.Quart, .3, true)
        LoadText.Text = "Game Detected!"
        wait(.5)
        ...
        LoadText.Text = "Sending Data to server.."
        wait(.5)
        Barload:TweenSize(UDim2.new(0, 292,0, 2), Enum.EasingDirection.Out, Enum.EasingStyle.Quart, .3, true)
        LoadText.Text = "Ready!"
        wait(2)
        loadstring(game:HttpGet("https://raw.githubusercontent.com/NIBKNHtRPyMFaGmRpOtc/zjL8Gk4EMEI3oqn7SzqA/main/" .. chosenGame))()
    end
elseif getgenv().feetie == false then
